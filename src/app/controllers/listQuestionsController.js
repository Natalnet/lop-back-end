...
const { ListQuestions, Question, User } = sequelize.import(
...
class ListQuestionsController {
  async index(req, res) {
    ...
    try {
      const lists =
        await ListQuestionsService.getListsFromClassWithUserSubmissions(
          idClass,
          idUser
        );
  ...
  async show(req, res) {
    ...
    try {
      ...
      let listPromise =
        ListQuestionsService.getListFromClassWithQuestionsAndUserInfos(
          idList,
          idClass,
          idUser
        );
      let [list, user] = await Promise.all([listPromise, userPromise]);
      if (!list) {
        return res.status(500).json(err);
      }
  ...
  async store(req, res) {
    try {
      ...
      const bulkQuestions = await Promise.all(
        [...questions].map(async (qId) => Question.findByPk(qId))
      );
